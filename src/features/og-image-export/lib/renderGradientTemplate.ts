...
function resolveTextColor(color1: string, color2: string): string {
  ...
  const eitherLight = isLightColor(color1) || isLightColor(color2)
  if (bothLight) return '#1e293b'
  if (eitherLight) return '#f1f5f9'
  return '#f1f5f9'
}
...
export async function renderGradientTemplate(
  ...
  const radialGrad = ctx.createRadialGradient(W * 0.3, H * 0.3, 0, W * 0.5, H * 0.5, W * 0.7)
  ...
  const subtitleColor = isLightColor(color1) && isLightColor(color2) ? '#475569' : '#cbd5e1'
  ...
  if (config.logoDataUrl) {
    await loadLogo(ctx, config.logoDataUrl, W / 2 - logoW / 2, logoY, logoW, logoH)
  }
