...
describe("renderCodeSnippetTemplate", () => {
  ...
  it("에디터 프레임 배경을 그린다 (수동 roundRectPath 호출)", async () => {
  ...
  it("title 텍스트를 그린다", async () => {
    const { drawAutoSizedText } = jest.requireMock(
      "../lib/renderOgImageToCanvas",
    );
    const ctx = makeMockCtx();
    const canvas = makeCanvas(ctx);
    ...
    const titleCall = calls.find(
      (args: unknown[]) => args[1] === "Hello World",
    );
  ...
  it("subtitle 텍스트를 그린다", async () => {
    ...
    const calls = (drawAutoSizedText as jest.Mock).mock.calls;
    const subtitleCall = calls.find(
      (args: unknown[]) => args[1] === "A subtitle here",
    );
  ...
  it("light 테마에서도 정상 동작한다", async () => {
    ...
    // roundRectPath와 arc는 테마에 무관하게 호출되어야 한다
    expect(ctx.moveTo).toHaveBeenCalled();
    expect(ctx.arc).toHaveBeenCalledTimes(3);
  });
  ...
  it("codeTheme이 없으면 dark 테마를 기본으로 사용한다", async () => {
    ...
    await expect(
      renderCodeSnippetTemplate(canvas, {
        ...baseConfig,
        codeTheme: undefined,
      }),
